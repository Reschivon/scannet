...
BASE_DIR="/workspace/scans"
DEST_DIR='.'

# Download ScanNet data
# python scannetlib/download_scannetv2.py -o /workspace/

# Define the start scene
START_SCENE="scene0000_00"
...
for SCENE_DIR in $(ls -d ${BASE_DIR}/scene* | sort); do
    ...
    if $START_PROCESSING; then
        ...
        # python scannetlib/reader.py --filename "${SCENE_DIR}/${SCENE_NAME}.sens" --output_path "scenes/${SCENE_NAME}_data"
        ...
        ./scannetlib/copy_files.sh "${SCENE_DIR}/${SCENE_NAME}_2d-label-filt/label-filt" "scenes/${SCENE_NAME}_data/labels"
    fi
done
